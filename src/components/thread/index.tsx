...
function ScrollToBottom(props: { className?: string }) {
  ...
  return (
    <Button
      className={cn("bg-white border border-gray-300 hover:bg-gray-100 focus:outline-none", props.className)}
...
export function Thread() {
  ...
  const { audioApiUrl } = useWidgetConfig(); // Added: Get audioApiUrl from config

  useEffect(() => {
    // When the Thread component mounts (e.g., chat window is opened or reopened),
    // ensure the chat history sidebar is collapsed.
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
      .then(audioStream => {
        ...
        mediaRecorder.onstop = () => {
          ...
          // Check if audioApiUrl is configured
          if (!audioApiUrl) {
            console.error("Audio API URL is not configured. Cannot transcribe audio.");
            // Optionally, provide user feedback, e.g., using toast.error()
            return; // Stop processing if URL is not available
          }
          ...
          // Enviar al backend
          console.log(`🚀 Enviando audio al endpoint de transcripción: ${audioApiUrl}`); // Use configured URL
          fetch(audioApiUrl, { // Use configured URL
            method: 'POST',
            body: formData,
          })
          .then(response => {
          ...
          <div className="flex items-center justify-between gap-2 p-1 z-10 relative">
            <div className="flex items-center justify-start gap-1 relative">
              <div className="absolute left-0 z-10">
                {(!chatHistoryOpen || !isLargeScreen) && (
                  <Button
                    className="hover:bg-gray-100 p-1 focus:outline-none"
                    variant="ghost"
                    onClick={() => setChatHistoryOpen((p) => !p)}
                  >
                    {chatHistoryOpen ? (
            ...
            <div className="flex items-center gap-2 pt-2 pr-2">
              {/* <TooltipIconButton
                ...
                onClick={handleNewThread}
              > */}
                <SquarePen className="size-4 cursor-pointer" onClick={handleNewThread} />
              {/* </TooltipIconButton> */}
            </div>
                    ...
                    <div className="flex items-center justify-end p-2 pt-2">
                      {stream.isLoading ? (
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            type="button"
                            className={`transition-all focus:outline-none ${isListening ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-transparent'}`}
                            size="sm"
                          ...
                          <Button
                            type="submit"
                            className="transition-all shadow-sm border border-gray-300"
